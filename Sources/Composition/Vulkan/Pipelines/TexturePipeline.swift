...
struct TextureVertexData {
    ...
    static func attributeDescriptions(binding: UInt32) -> [VkVertexInputAttributeDescription] {
        [
            // x, y, w, h
            .init(
                location: 0,
            ...
            .init(
                location: 1,
                binding: binding,
                format: VK_FORMAT_R32G32B32A32_UINT,  // 4 Int
                offset: UInt32(MemoryLayout<Self>.offset(of: \.textureData)!)
            ),
            .init(
                location: 2,
                binding: binding,
...
class TexturePipeline {
    ...
    init(
        ...
        self.device = device
        let vertexShader = Result {
            try Shader(filename: "texture.vert.spv", device: device)
        }.expect("Cannot create shader")
        let fragmentShader = Result {
            try Shader(filename: "texture.frag.spv", device: device)
        }.expect("Cannot create shader")
        ...
        let fragName = CString("main")
        fragCi.pName = fragName.ptr

        let shaderStages = [vertCi, fragCi]

        let bindingDescriptions = Pin([
            TextureVertexData.bindingDescriptions(binding: 0)
        ])
        let attributeDescriptions = Pin(TextureVertexData.attributeDescriptions(binding: 0))
