...
class MainPipeline {
    ...
    init(
        ...
        let vertexShader = Result {
            try Shader(filename: "rounded_rectangle.vert.spv", device: device)
        }.expect("Cannot create shader")
        let fragmentShader = Result {
            try Shader(filename: "rounded_rectangle.frag.spv", device: device)
        }.expect("Cannot create shader")
        ...
        let attributeDescriptions = Pin(RoundedRectangleVertexData.attributeDescriptions)
        let bindingDescriptions = Pin([RoundedRectangleVertexData.bindingDescriptions])
