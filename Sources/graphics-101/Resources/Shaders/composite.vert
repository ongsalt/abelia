...
layout(location = 0) in vec4 inOpacityAndScreenSize;
...
layout(location = 0) flat out vec4 outOpacityAndScreenSize;
...
layout(location = 8) out vec4 outFillColor;
layout(location = 9) out vec4 outBorderColor;
layout(location = 10) out vec4 outShadowColor;
layout(location = 11) out vec4 outShadow; // TODO: normalized this from the cpu

layout(location = 12) out uvec4 outContents;
layout(location = 13) out vec4 outNineGrid;


void main() {
    outOpacityAndScreenSize = inOpacityAndScreenSize;
    outSizing = inSizing;
    ...
    vec2 scaled_pos = (vertexPos / vec2(inOpacityAndScreenSize.y, inOpacityAndScreenSize.z)) * 2.0 - 1.0;
